...
#[tokio::test]
async fn subscribe_returns_a_200_when_field_present_but_empty() {
    let app = spawn_app().await;

    let test_cases = vec![
        ("name=&email=kevin%40gmail.com", "empty name"),
        ("name=kein&email=", "empty email"),
        ("name=kevin&emmail=not-a-valid-email", "invalid email"),
    ];
